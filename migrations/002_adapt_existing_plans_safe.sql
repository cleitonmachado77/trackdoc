-- Migration: Adaptar Tabelas Existentes para o Sistema de Planos (VERSÃO SEGURA)
-- Esta migration adapta as tabelas existentes de forma segura
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'plans' AND column_name = 'type') THEN
    ALTER TABLE plans ADD COLUMN type VARCHAR(50);
  END IF;
  
  -- Adicionar interval
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'plans' AND column_name = 'interval') THEN
    ALTER TABLE plans ADD COLUMN interval VARCHAR(20) DEFAULT 'monthly';
  END IF;
  
  -- Adicionar stripe_price_id
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'plans' AND column_name = 'stripe_price_id') THEN
    ALTER TABLE plans ADD COLUMN stripe_price_id VARCHAR(255);
  END IF;
  
  -- Adicionar stripe_product_id
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'plans' AND column_name = 'stripe_product_id') THEN
    ALTER TABLE plans ADD COLUMN stripe_product_id VARCHAR(255);
  END IF;
  
  -- Adicionar usuario_adicional_preco
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'plans' AND column_name = 'usuario_adicional_preco') THEN
    ALTER TABLE plans ADD COLUMN usuario_adicional_preco DECIMAL(10, 2);
  END IF;
  
  -- Adicionar armazenamento_extra_preco
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'plans' AND column_name = 'armazenamento_extra_preco') THEN
    ALTER TABLE plans ADD COLUMN armazenamento_extra_preco DECIMAL(10, 2);
  END IF;
END $$;
...
BEGIN
  -- Adicionar entity_id
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'subscriptions' AND column_name = 'entity_id') THEN
    ALTER TABLE subscriptions ADD COLUMN entity_id UUID REFERENCES entities(id) ON DELETE SET NULL;
  END IF;
  ...
  -- Adicionar current_users
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'subscriptions' AND column_name = 'current_users') THEN
    ALTER TABLE subscriptions ADD COLUMN current_users INTEGER DEFAULT 1;
  END IF;
  
  -- Adicionar current_storage_gb
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'subscriptions' AND column_name = 'current_storage_gb') THEN
    ALTER TABLE subscriptions ADD COLUMN current_storage_gb DECIMAL(10, 2) DEFAULT 0;
  END IF;
  
  -- Adicionar canceled_at
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
  ...
  -- Adicionar trial_start_date (se não existir)
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'subscriptions' AND column_name = 'trial_start_date') THEN
    ALTER TABLE subscriptions ADD COLUMN trial_start_date TIMESTAMP WITH TIME ZONE;
  END IF;
  
  -- Adicionar trial_end_date (se não existir)
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
...
-- Migrar dados de trial_start para trial_start_date (se necessário)
DO $$ 
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns 
             WHERE table_name = 'subscriptions' AND column_name = 'trial_start') THEN
    UPDATE subscriptions 
    SET trial_start_date = trial_start 
    WHERE trial_start_date IS NULL AND trial_start IS NOT NULL;
  END IF;
...
-- Atualizar plano Trial (se existir e type for NULL)
UPDATE plans 
SET type = 'trial',
    interval = 'monthly',
...
-- ============================================================================
-- 7. PREENCHER DADOS FALTANTES EM SUBSCRIPTIONS EXISTENTES
-- ============================================================================
...
BEGIN
  ...
  -- Verificar funções
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'get_user_active_subscription') THEN
    RAISE NOTICE '✓ Função get_user_active_subscription criada';
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'create_trial_subscription') THEN
    RAISE NOTICE '✓ Função create_trial_subscription criada';
  END IF;
