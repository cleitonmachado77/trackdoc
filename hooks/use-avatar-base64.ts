...
export function useAvatarBase64() {
  const { user } = useAuth()
    ...
    try {
      ...
      if (error.message?.includes('fetch failed') || error.message?.includes('timeout')) {
        ...
        toast({
          ...
          description: error.message || "Não foi possível atualizar sua foto de perfil. Tente novamente.",
          variant: "destructive",
        })
      }
      return null
  ...
  const deleteAvatar = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: error.message || "Não foi possível remover sua foto de perfil. Tente novamente.",
        variant: "destructive",
      })
      return false
    } finally {
