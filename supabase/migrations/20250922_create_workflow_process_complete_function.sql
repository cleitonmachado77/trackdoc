-- Função RPC personalizada que cria processo e execuções contornando RLS
CREATE OR REPLACE FUNCTION create_workflow_process_complete(
...
BEGIN
  ...
  FOR v_step IN 
    ...
    ORDER BY step_order
  LOOP
    -- 4. Gerar ID único para cada execução
    v_execution_id := gen_random_uuid();
    
    -- 5. Inserir execução na tabela workflow_executions
  ...
  -- 6. Retornar ID do processo criado
  RETURN v_process_id;
END;
$$;
