...
-- 8. FUNÇÃO MELHORADA PARA CRIAR PERFIL AUTOMATICAMENTE
...
-- 12. RESTAURAR DADOS DO BACKUP (se existir) - Versão robusta
DO $$
DECLARE
    backup_exists BOOLEAN := FALSE;
    has_company BOOLEAN := FALSE;
    has_registration_type BOOLEAN := FALSE;
    has_entity_role BOOLEAN := FALSE;
    has_registration_completed BOOLEAN := FALSE;
    has_selected_plan_id BOOLEAN := FALSE;
BEGIN
    ...
    IF backup_exists THEN
        ...
        SELECT EXISTS (
            ...
            WHERE table_name = 'profiles_backup' AND column_name = 'company'
        ) INTO has_company;
        
        SELECT EXISTS (
            SELECT 1 FROM information_schema.columns 
            WHERE table_name = 'profiles_backup' AND column_name = 'registration_type'
        ) INTO has_registration_type;
        
        SELECT EXISTS (
            SELECT 1 FROM information_schema.columns 
            WHERE table_name = 'profiles_backup' AND column_name = 'entity_role'
        ) INTO has_entity_role;
        
        SELECT EXISTS (
            SELECT 1 FROM information_schema.columns 
            WHERE table_name = 'profiles_backup' AND column_name = 'registration_completed'
        ) INTO has_registration_completed;
        
        SELECT EXISTS (
            SELECT 1 FROM information_schema.columns 
            WHERE table_name = 'profiles_backup' AND column_name = 'selected_plan_id'
        ) INTO has_selected_plan_id;
        
        -- Restaurar dados com base nas colunas disponíveis
        IF has_company AND has_registration_type AND has_entity_role THEN
            -- Backup completo - restaurar tudo
            INSERT INTO profiles (
                id, full_name, email, phone, company, role, status, 
                permissions, avatar_url, entity_id, department_id, position,
                last_login, registration_type, entity_role, registration_completed,
                selected_plan_id, created_at, updated_at
            )
            SELECT 
                ...
                COALESCE(permissions, '["read", "write"]'),
                avatar_url, entity_id, department_id, position,
                last_login, 
                CASE WHEN has_registration_type THEN COALESCE(registration_type, 'individual') ELSE 'individual' END,
                CASE WHEN has_entity_role THEN COALESCE(entity_role, 'user') ELSE 'user' END,
                CASE WHEN has_registration_completed THEN COALESCE(registration_completed, true) ELSE true END,
                CASE WHEN has_selected_plan_id THEN selected_plan_id ELSE NULL END,
                created_at, updated_at
            FROM profiles_backup
            ON CONFLICT (id) DO NOTHING;
        ELSE
            -- Backup básico - restaurar apenas campos essenciais
            INSERT INTO profiles (
        ...
        RAISE NOTICE 'Dados restaurados do backup (company: %, reg_type: %, entity_role: %)', 
                     has_company, has_registration_type, has_entity_role;
    ELSE
        RAISE NOTICE 'Nenhum backup encontrado para restaurar';
    END IF;
END $$;
