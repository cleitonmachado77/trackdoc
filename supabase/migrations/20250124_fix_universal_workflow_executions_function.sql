-- Correção da função RPC universal para criar execuções de workflow
-- Problema: ambiguidade na coluna "assigned_to" entre variável PL/pgSQL e coluna da tabela
...
DROP FUNCTION IF EXISTS criar_execucoes_workflow_universal(uuid,uuid,uuid);
DROP FUNCTION IF EXISTS criar_execucoes_workflow_universal(uuid,uuid);
...
BEGIN
  ...
  LOOP
    ...
    -- Retornar dados da execução criada (usando nomes de variáveis únicos)
