...
-- 5. ATIVAR USUÁRIOS QUE JÁ CONFIRMARAM EMAIL MAS ESTÃO PENDENTES
UPDATE public.profiles 
SET 
    status = 'active',
    registration_completed = true,
...
SELECT 
    ...
    COUNT(CASE WHEN p.status = 'pending_email' AND u.email_confirmed_at IS NULL THEN 1 END) as usuarios_pendentes,
    COUNT(CASE WHEN p.status = 'active' AND u.email_confirmed_at IS NOT NULL THEN 1 END) as usuarios_ativos,
    COUNT(CASE WHEN p.status = 'active' AND u.email_confirmed_at IS NULL THEN 1 END) as usuarios_ativos_sem_confirmacao
FROM public.profiles p
JOIN auth.users u ON p.id = u.id;