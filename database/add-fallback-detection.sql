-- 🔧 CORREÇÃO: Adicionar detecção de fallback para entidades

-- Modificar a função handle_new_user para detectar entidades mesmo sem metadata correto
...
BEGIN
    ...
    IF NEW.raw_user_meta_data IS NOT NULL THEN
        ...
        -- FALLBACK: Detectar se tem dados de entidade mesmo com registration_type errado
        IF (v_entity_name IS NOT NULL AND v_entity_name != '') OR 
           ...
           (v_entity_cnpj IS NOT NULL AND v_entity_cnpj != '') THEN
            ...
            v_role := 'admin';
            v_entity_role := 'admin';
            RAISE NOTICE '🔧 [handle_new_user] FALLBACK: Detectados dados de entidade, corrigindo registration_type';
        END IF;
    END IF;
    ...
    RAISE NOTICE '📋 [handle_new_user] Dados finais: Nome=%, Role=%, Tipo=%, HasEntityData=%', 
                 v_full_name, v_role, v_registration_type, v_has_entity_data;
    ...
    RAISE NOTICE '✅ [handle_new_user] Perfil criado para: % (Tipo: %)', NEW.email, v_registration_type;
...
DO $$
BEGIN
    RAISE NOTICE '✅ Função handle_new_user atualizada com detecção de fallback';
    RAISE NOTICE '🔧 Agora detecta entidades mesmo se registration_type estiver errado';
    RAISE NOTICE '🧪 Teste registrando nova entidade';
END $$;